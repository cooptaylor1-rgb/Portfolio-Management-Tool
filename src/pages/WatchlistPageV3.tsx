/**
 * Watchlist Page V3 - Professional Column-Customizable Securities Monitor
 * 
 * Features:
 * - Fully customizable columns with 60+ available metrics
...
interface WatchlistItem {
  ...
  avgVolume?: number;
  priceHistory?: number[];
  // Extended fields (mostly placeholder for future data)
  industry?: string;
  country?: string;
}
...
function Sparkline({ data, positive }: { data?: number[]; positive: boolean }) {
  if (!data || data.length < 2) return <span className="no-data">—</span>;
...
function CellRenderer({
  ...
  column: WatchlistColumnDef;
  item: WatchlistItem;
  onToggleAlert: (id: string) => void;
}) {
  const dataField = column.dataField || column.id;
  const value = (item as any)[dataField];
  ...
  switch (column.id) {
    ...
    case 'sector':
      return item.sector ? <span>{item.sector}</span> : <span className="no-data">—</span>;
      
    case 'marketCap':
      return item.marketCap ? <span>{item.marketCap}</span> : <span className="no-data">—</span>;
      
    case 'price':
      return <span className="price-value">${item.price.toFixed(2)}</span>;
      
    case 'priceChange1D':
      return (
        <div className={`change-info ${item.change >= 0 ? 'positive' : 'negative'}`}>
    ...
    case 'sparkline':
      return <Sparkline data={item.priceHistory} positive={item.change >= 0} />;
      
    case 'volume':
      return item.volume ? (
        <span>{(item.volume / 1_000_000).toFixed(1)}M</span>
      ) : <span className="no-data">—</span>;
      
    case 'avgVolume10D':
      return item.avgVolume ? (
        <span>{(item.avgVolume / 1_000_000).toFixed(1)}M</span>
      ) : <span className="no-data">—</span>;
      
    case 'targetPriceUser': {
      if (!item.targetPrice) return <span className="no-data">—</span>;
      const upside = ((item.targetPrice - item.price) / item.price) * 100;
      return (
    ...
    case 'conviction': {
      const level = CONVICTION_LEVELS.find(l => l.value === item.conviction);
      return (
        <span 
    ...
    case 'alertEnabled':
      return (
        <button 
          ...
          onClick={(e) => { e.stopPropagation(); onToggleAlert(item.id); }}
          title={item.alertEnabled ? 'Alert enabled' : 'Alert disabled'}
        >
    ...
    case 'addedAt':
      return <span className="date-value">{item.addedAt}</span>;
      
    default:
      // For unavailable columns, show placeholder
      if (!column.available) {
        return <span className="no-data coming-soon" title="Coming soon">—</span>;
      }
      ...
      if (typeof value === 'number') {
        if (column.percentage) {
          return <span>{value >= 0 ? '+' : ''}{value.toFixed(column.decimals ?? 2)}%</span>;
        }
        if (column.currency) {
          return <span>${value.toFixed(column.decimals ?? 2)}</span>;
        }
        return <span>{value.toFixed(column.decimals ?? 2)}</span>;
      }
...
export default function WatchlistPage() {
  ...
  const [preferences, setPreferences] = useState<WatchlistPreferences>(() => 
    loadWatchlistPreferences()
  );
  ...
  // Get active columns based on preferences
  const activeColumns = useMemo(() => {
    const orderedIds = getOrderedVisibleColumns(preferences);
    return orderedIds
      .map(id => getColumnById(id))
      .filter((col): col is WatchlistColumnDef => col !== undefined);
  }, [preferences]);
  ...
  const stats = useMemo(() => {
    ...
    return { withAlerts, highConviction, avgUpside };
  }, [watchlist]);
  ...
  const filteredWatchlist = useMemo(() => {
    ...
    if (preferences.sortBy) {
      const { id, direction } = preferences.sortBy;
      filtered.sort((a, b) => {
        ...
        switch (id) {
          ...
          case 'volume':
            ...
            break;
          case 'marketCap':
            // Parse market cap string (e.g., "$790B")
            const parseMktCap = (s?: string) => {
              if (!s) return 0;
            ...
            aVal = parseMktCap(a.marketCap);
            bVal = parseMktCap(b.marketCap);
            break;
          default:
            return 0;
        }
    ...
    return filtered;
  }, [watchlist, searchTerm, preferences.sortBy, convictionFilter, thesisFilter]);

  // Handlers
  const handleSort = useCallback((columnId: WatchlistColumnId) => {
    const col = getColumnById(columnId);
    if (!col?.sortable) return;
    
    setPreferences(prev => {
      const updated = updateSort(prev, columnId);
      saveWatchlistPreferences(updated);
      return updated;
    });
  }, []);

  const handleSavePreferences = useCallback((newPrefs: WatchlistPreferences) => {
    setPreferences(newPrefs);
    saveWatchlistPreferences(newPrefs);
  }, []);
  ...
  const exportCSV = useCallback(() => {
    ...
    const rows = filteredWatchlist.map(item => 
      activeColumns.map(col => {
        const field = col.dataField || col.id;
  ...
  }, [filteredWatchlist, activeColumns]);
  ...
  // Render sort icon
  const SortIcon = ({ columnId }: { columnId: WatchlistColumnId }) => {
    const col = getColumnById(columnId);
    if (!col?.sortable) return null;
    
    if (preferences.sortBy?.id !== columnId) {
          ...
          <thead>
            <tr>
              ...
              {activeColumns.map(col => (
                <th 
                  key={col.id}
                  className={`watchlist-table__col watchlist-table__col--${col.id}`}
                  style={{ 
              ...
              >
                ...
                {activeColumns.map(col => (
                  <td 
                    ...
                    className={`watchlist-table__cell watchlist-table__cell--${col.id}`}
                  >
                    <CellRenderer
                      column={col}
                      item={item}
